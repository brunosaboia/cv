...
2. Resolves cross-references and derived fields **once**, before any template runs, so every layout reads the same plain, pre-resolved values instead of re-deriving them:
   - `experience[]` is normalized into one company node per employer holding a `roles` list (`normalize_experience`), **before** company resolution below, which therefore still sees exactly one node per entry and needs no change. The older flat spelling (role fields directly on the entry) collapses to a one-role group here, so no template branches on which shape the JSON used and a flat entry renders byte-identically to how it did before `roles` existed — required, since `cv-data`'s deploy checks this repo out unpinned. All three layouts then re-expand the group with the `flatten_roles` filter, rendering one self-contained block per role (title, employer, website, body) so the employer repeats per role — `classic` nested them under a shared header until the title moved to the block's first line, which left no position for such a header — see `README.md`'s "Several roles at one employer".
   ...
   - `languages[].level` (an integer `0`–`6`: native plus CEFR C2→A1, market-independent) is translated to a display string (`descriptive` scale, or `cefr` for `CH`/`DE`) via `LANGUAGE_LEVEL_LABELS`.
