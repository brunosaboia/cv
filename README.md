...
That grouping is a fact about the data, not about the page. All three layouts **re-expand** it with the `flatten_roles` filter and render one self-contained block per role — title, employer and location, website, then that role's own body — so a promotion prints as two blocks that repeat the employer. For `ats` and `txt` the reason is parsing: a résumé parser keys on a title/company/dates triple appearing on consecutive lines, and a lone header with two roles under it is exactly the shape that makes it attribute both to the first title, or drop the earlier role — the failure the `ats` layout exists to avoid. `classic` used to nest roles under a shared company header instead; once the title moved to the block's first line there was nowhere left for such a header to go, since a header can only precede what it heads. What that gives up is the employer's combined tenure across the group, which no longer has a line of its own to sit on: each block states its own dates and duration.
